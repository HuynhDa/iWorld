...
const Admin = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (!user || user.role !== 0) {
      // Nếu không phải là admin (role !== 0), chuyển hướng về trang đăng nhập
      navigate('/login');
      toast.error("Bạn cần đăng nhập đúng tài khoản để vào trang Admin");
    } else if (user.role === 1) {
      // If role is 1 (non-admin), show alert
      setShowAlert(true);
    }
  }, [navigate]);
      ...
      <Row>
        ...
        <Col md={2} className="bg-dark text-white p-2" style={{ minHeight: '100vh' }}>
          ...
          <Nav variant="pills" className="flex-column">
            ...
            <Nav.Item className="mb-5">
              <Link to="/voucher" className="nav-link text-white" style={{ fontWeight: 'bold' }}>
                <FaComments className="me-2" /> Voucher
              </Link>
            </Nav.Item>
            <Nav.Item className="mb-5">
